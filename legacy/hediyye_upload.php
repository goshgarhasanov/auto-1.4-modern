...
{
echo "<div class=\"sms\">\n";
echo "<form ENCTYPE=\"multipart/form-data\" action=\"hediyye_upload.php?id=$id&amp;ps=$ps&amp;ref=$ref\" method=\"post\">\n";
print 'Fayl hediyye:<br/>';
print $_v->input("<input name=\"file\" type=\"file\"/>")."<br/>\n";
print 'Kategoriya:<br/>';
$option = "<select name=\"dir\">|";
$dir = opendir("hediyye");
$i=1;
...
{
$option .= '<option value="'.$file.'">'.$file.'('.count_files('hediyye/'.$file).')</option>|';
}
++$i;
}
closedir($dir);
   ...
   $is_file = $_FILES['file']['tmp_name'];
	if(!is_uploaded_file($is_file)){
		$albom->error = 'Fayl&#305; Se&#231;memisiz.';
			...
			if($FileSize > 300 * 1024) { // 500 kb
				$albom->error = '&#350;ekil 300 kb-dan &#231;ox olmamal&#305;d&#305;r!';
...
echo '---</div>';
echo "<a href=\"hediyye_upload.php?id=$id&amp;ps=$ps&amp;ref=$ref\">Geri Qay&#305;t</a><br/>\n";
$_v->fsize2($fsize2);
$_v->end('1',$link);
exit();
}
...
elseif(200 <= $size[0])
{
$error = 'Fayl&#305;n &#231;ekisi 100-kb dan &#231;ox olmamal&#305;d&#305;r..!<br/>';
}
elseif($par[0] >= '200' or $par[1] >= '200')
{
$error = '&#350;eklin eni uzunu 200 x 200 den &#231;ox olmamal&#305;d&#305;r..!<br/>';
}
if($error!='')
{
print $error;
...
{
$count = count_files("hediyye");
$filen = ($count+1).".png";
if(copy($file, "hediyye/".$dir."/".$filen))
{
print 'Hediyye y&#252;klendi. &#304;D: '.($count+1).'<br/>';
print '<img src="hediyye/'.$dir.'/'.$filen.'" alt="Hediyye"><br/>';
...
echo "<a href=\"hediyye_upload.php?id=$id&amp;ps=$ps&amp;ref=$ref\">Geri Qayit</a><br/>";
}
}
$_v->divide('html');
echo "<a href=\"nihad_niko.php?nn=83&amp;id=$id&amp;ps=$ps&amp;ref=$ref\">Panel</a><br/>";
