...
switch ($bol)
{
	default:
	echo "Leqeb / &#304;D:<br/>";
	$_v->action("beyen.php?id=$id&amp;ps=$ps&amp;bol=add&amp;ref=$ref");
	print $_v->input("<input name=\"nick$ref\" title=\"nick\" emptyok=\"true\"/>").'<br/>';
	print $_v->submit1("Beyenirem ($beyen_b bal)")."<br/>";
	...
	mysql_query("INSERT INTO `zapiski` SET `idtowhom` = '".$oxu["id"]."',`towhom` = '".$oxu["user"]."',`idwho` = '0',`time` = '".$SERVER_TIME."',`who` = 'Sistem',`date` = '',`readd` = '0',`topic` = 'Beyendiklerim',`message` = '".$metn."';");
	mysql_query ("INSERT INTO beyen SET kim = '".$id."', kimi = '".$oxu["id"]."', vaxt = '".$SERVER_TIME."'");
