...
$banned = mysql_query ("Select * from `bannlist` WHERE (`ip` = '".$REMOTE_MAX_USER."')and(`soft` = '".$soft."');");
...
mysql_query("UPDATE `conf` SET `ipp` = '".$REMOTE_MAX_USER."', `soft` = '".$soft."';");
mysql_query ("INSERT INTO `bannlist` SET `ip` = '".$REMOTE_MAX_USER."', `soft` = '".$soft."', `user` = '".$pnik."', `moder` = '".$user."', `sebeb` = '".$whykik."';");
mysql_query("UPDATE `users` SET `time` = '".($SERVER_TIME-$_AUTO['ofline'])."', `banned` = '3' WHERE `user_ip`='".$ip."' and `user_soft` = '".$soft."' and `banned` = '0';");
...
$txt = "<b>$user_admin</b>, \"<u>$pnik</u>\" leqebli istifade&#231;inin <b><u>Telefon Modelini</u> - BAN Etdi</b>$whykikk\n";
for ($num = 0; $num <= 9; $num++){
$room = "room".$num;
