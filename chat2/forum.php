...
$online = time( ) + $vaxt;
mysql_query( "UPDATE `users` SET `time` = '".$online."', `place` = '40' WHERE `id` = '".$id."' LIMIT 1;" );
$uid = mysql_real_escape_string($_GET['uid']);
$tema = mysql_query( "SELECT * FROM `sh_tem` WHERE `id`='{$uid}'" );
...
if ( $cmd == "4" )
{
    ...
    $uid = $_GET['uid'];
    $tema = @mysql_query( @$Var_1320 );
    $tema_name_nemo = mysql_result(mysql_query( "SELECT name FROM sh_tem WHERE id='".intval($_GET["uid"])."'" ),0);
    $title = "M&#246;vzu";
    $title2 = "Forum / {$tema_name_nemo}";
}
else if ( $cmd == "3" )
{
    if ( !isset( $_GET['uid'] ) )
...
echo "<card id=\"forum\" title=\"{$title1}\">\n";
echo "<p align=\"left\">\n";
echo $fsize1;
...
default :
    ...
    if ( mysql_affected_rows( ) != 0 )
    {
        ...
        while ( $new = mysql_fetch_array( $new_q ) )
        {
            ...
            if ( $new['description'] != "" )
            {
                echo "<small>{$new['description']}</small><br/>\n";
            }
        }
        echo "----<br/>\n";
    }
    ...
    while ( $actual = mysql_fetch_array( $actual_q ) )
    {
        ...
        //$ddunen = mysql_fetch_array( "d.m.Y", mktime( date( "H" ) - 24 ) );
        ...
        if ( mysql_num_rows( $us_av ) == 1 )
        {
            echo "<a href=\"inside.php?id={$id}&amp;ps={$ps}&amp;&amp;nk=".$user_avtor['id']."&amp;ref={$ref}\">".$user_avtor['user']."</a>]<br/>";
        }
        else
        {
            echo "Silinib[".$posl_post['avtor']."]]<br/>\n";
        }
    }
    ...
    case "m" :
        ...
        if ( $posts <= 0 )
        {
            ...
            if ( $rm != "" )
            {
                echo "<a href=\"{$filechat}?{$mygetname}&amp;rm={$rm}&amp;nk={$nk}&amp;ref={$ref}\">Geri Qay&#305;t</a><br/>\n";
            }
        ...
        if ( 0 < $page - 1 )
        {
            $page1left = " <a href=\"".$url_for_pstr.( $page - 1 )."&amp;ref={$ref}\">".$Tmp_619."</a> | ";
        }
        if ( $page + 5 <= $total )
        {
    ...
    case "yeni" :
        ...
        if ( !$error )
        {
            if ( !isset( $_POST['name'] ) && !isset( $_POST['text'] ) )
            {
                ...
                else if ( $dat_fkecersiz == 2 )
                {
                    if ( $forum_level <= 1 && $row['level'] <= 3 )
                    {
                        $ballazim = 1;
                    }
                }
                else if ( $dat_fkecersiz == 3 )
                {
                    $ballazim = 0;
                }
                else if ( $dat_fkecersiz == 4 )
                {
                    $ballazim = 1;
                }
                else
                ...
                echo "M&#246;vzunun ad&#305;:<br/><input name=\"name{$ref}\" maxlength=\"15\" emptyok=\"true\"/><br/>";
                echo "M&#246;vzu:<br/><input name=\"text{$ref}\" maxlength=\"1000\" emptyok=\"true\"/><br/>";
                echo "<anchor>Yarat<go href=\"forum.php?{$mygetname}&amp;cmd=yeni&amp;cd={$cd}&amp;uid={$uid}&amp;ref={$ref}\" method=\"post\">\r\n<postfield name=\"name\" value=\"$(name{$ref})\"/>\r\n<postfield name=\"text\" value=\"$(text{$ref})\"/>\r\n</go></anchor><br/>----<br/>\n";
            ...
            else
            {
                ...
                {
                    $error = "M&#246;vzunu &#231;ox q&#305;sa yazm&#305;s&#305;z, M&#246;vzunun metini &#231;ox olmal&#305;d&#305;r!<br/>";
                }
                if ( 60 < strlen( $_POST['name'] ) )
                {
                    $error = "M&#246;vzunu ad&#305; 50 herfden &#231;ox olmamal&#305;d&#305;r!<br/>";
                }
                if ( $_POST['name'] == "" )
                {
                    $error = "M&#246;vzunun ad&#305;n&#305; yazmam&#305;s&#305;z.<br/>";
                }
                if ( $_POST['text'] == "" )
                {
                    $error = "M&#246;vzunu yazmam&#305;s&#305;z.<br/>";
                }
                if ( $id == $s_u_id )
                {
                ...
                if ( !$error )
                {
                    ...
                    if ( $minpos != 355 )
                    {
                        $st1 = substr( $text, 0, $minpos + strlen( $smiles[$nm] ) );
                        $st2 = substr( $text, $minpos + strlen( $smiles[$nm] ), strlen( $text ) - strlen( $st1 ) );
                    ...
                    $date = date( "d.m.Y H:i" );
                    $time = time( );
                    $add_topic = mysql_query( "INSERT INTO `sh_tem` SET name='{$name}', `time`='{$time}', `cat`='{$uid}', `avtor`='{$id}', `tesdiq`='{$dat_ftesdiq}';" );
                    if ( $add_topic )
                    {
                        $updatepodforum = "";
                        $id_topic = mysql_insert_id( );
                        $add_post = mysql_query( "INSERT INTO `sh_post` SET avtor='{$id}', date='{$date}', text='{$text}', tema='{$id_topic}', `time`='{$time}';" );
                        ...
                        if ( $dat_ftesdiq == 2 )
                        {
                            $updatepodforum = "`fpost`='{$plus_post_me}'";
                        }
                        if ( $updatepodforum != "" )
                        {
                        ...
                        if ( $add_post )
                        {
                            echo "Sizin Qeyd etdiyiniz m&#246;vzu yarad&#305;ld&#305;.<br/>\n";
                        }
                        if ( $dat_ftesdiq != 2 )
                        {
                            echo "Forum idarecileri Sizin M&#246;vzunu tesdiqlemesini g&#246;zleyin<br/>\n";
                        }
                        if ( $add_post )
                        {
                            echo "----<br/><a href=\"forum.php?{$mygetname}&amp;cmd={$cd}&amp;uid={$uid}&amp;ref={$ref}\">B&#246;lmeye Qay&#305;t</a><br/>\n";
                        }
                    }
                    else
                    {
                        echo "Xeta <br/>----<br/><a href=\"forum.php?{$mygetname}&amp;cmd=yeni&amp;cd={$cd}&amp;uid={$uid}{$ref}\">Geri Qay&#305;t</a><br/>\n";
                    }
                }
                else
                {
                    echo $error;
                    echo "----<br/>M&#246;vzunun ad&#305;:<br/><input name=\"name{$ref}\" value=\"".$_POST['name']."\" maxlength=\"50\" emptyok=\"true\"/><br/>";
                    echo "M&#246;vzu:<br/><input name=\"text{$ref}\" maxlength=\"1000\" value=\"".$_POST['text']."\" emptyok=\"true\"/><br/>";
                    echo "<anchor>Yarat<go href=\"forum.php?{$mygetname}&amp;cmd=yeni&amp;cd={$cd}&amp;uid={$uid}&amp;ref={$ref}\" method=\"post\">\r\n<postfield name=\"name\" value=\"$(name{$ref})\"/>\r\n<postfield name=\"text\" value=\"$(text{$ref})\"/>\r\n</go></anchor><br/>----<br/>\n";
                    echo "<a href=\"forum.php?{$mygetname}&amp;cmd={$cd}&amp;uid={$uid}&amp;ref={$ref}\">Geri Qay&#305;t</a><br/>----<br/>\n";
                }
            }
        ...
        {
            echo $error;
            echo "----<br/><a href=\"forum.php?{$mygetname}&amp;cmd=yeni&amp;cd={$cd}&amp;uid={$uid}&amp;ref={$Var_30936}\">Geri Qay&#305;t</a><br/>----<br/>\n";
        }
        echo "<a href=\"forum.php?{$mygetname}&amp;ref={$ref}\">Forum</a> |\n";
        break;
    case "1" :
        ...
        if ( isset( $_GET['edit'] ) && 3 <= $forum_level )
        {
            $uid = $_GET['uid'];
            if ( !( $new = mysql_fetch_array( mysql_query( "SELECT * FROM `sh_new` WHERE `id`='{$Var_31416}'" ) ) ) )
            {
            ...
            if ( !isset( $_POST['name'] ) )
            {
                ...
                echo $fsize2;
                echo "<input name=\"name{$ref}\" value=\"{$new['name']}\"  maxlength=\"100\" emptyok=\"true\"/><br/>\n";
                echo $fsize1;
                echo "<u>Text S&#246;z</u><br/>\n";
                echo $fsize2;
                echo "<input name=\"desc{$ref}\" value=\"{$new['description']}\"  maxlength=\"1000\" emptyok=\"true\"/><br/>\n";
                echo $fsize1;
                echo "<i>Xeberin mezmunu</i><br/>\n";
                echo $fsize2;
                echo "<input name=\"text{$ref}\" value=\"{$new['text']}\"  maxlength=\"400\" emptyok=\"true\"/><br/>\n";
                echo $fsize1;
                echo "<anchor>Elave et<go href=\"forum.php?{$mygetname}&amp;cmd=1&amp;uid={$uid}&amp;edit\" method=\"post\">\r\n<postfield name=\"name\" value=\"$(name{$ref})\"/>\r\n<postfield name=\"desc\" value=\"$(desc{$ref})\"/>\r\n<postfield name=\"text\" value=\"$(text{$ref})\"/>\r\n</go></anchor><br/>\n";
            }
            else if ( $_POST['text'] != "" && $_POST['name'] != "" )
            {
                ...
                {
                    echo mysql_error( )."Xeta";
                }
            }
            echo "----<br/><a href=\"forum.php?{$mygetname}&amp;ref={$ref}\">Forum</a>\n";
        }
        else if ( isset( $_GET['del'] ) && 3 <= $forum_level )
        {
            $uid = $_GET['uid'];
            ...
            if ( mysql_query( "DELETE FROM `sh_new` WHERE `id`='{$uid}'" ) )
            {
                echo "Xeber silindi<br/>----<br/>\n";
            }
            else
            {
                echo mysql_error( )."Xeta<br/>----<br/>";
            }
            echo "<a href=\"forum.php?{$mygetname}&amp;ref={$ref}\">Forum</a>\n";
        }
        else
            ...
            if ( @mysql_fetch_array( @mysql_query( @"SELECT * FROM `sh_new` WHERE `id`='{$uid}';" ) ) )
            {
                if ( mysql_num_rows( $new_q ) != 0 )
                {
                    ...
                    while ( $new = mysql_fetch_array( $new_q ) )
                    {
                        ...
                        if ( !$user_avtor )
                        {
                            $avtor = "Silinib.".$new['avtor'];
                        }
                        else
                        {
                            $avtor = "<a href=\"inside.php?{$mygetname}&amp;nk={$user_avtor['id']}&amp;ref={$ref}\">{$user_avtor['user']}</a>\n";
                        }
                        echo $fsize2."</p><p align=\"center\"><b>{$new['name']}</b></p><p>{$fsize1} ".shch( $new['text'] )."<br/>-------<br/>M&#252;ellif: {$avtor}<br/>\n";
                    }
                    if ( 3 <= $forum_level )
                    {
                        echo "<a href=\"forum.php?{$mygetname}&amp;cmd=1&amp;uid={$uid}&amp;edit&amp;ref={$ref}{$mygetname}&amp;cmd=1&amp;uid={$uid}&amp;del\">Delete</a><br/>----<br/>\n";
                    }
                }
            }
            else
   ...
   case "4" :		
	...
	if ( $_POST['text'] != "" )
    {
        $tema_q = mysql_query( "SELECT * FROM `sh_new` WHERE `id`='{$uid}'" );
        if ( !$tema_q )
        {
            $errormesaj .= "M&#246;vzu tap&#305;lmad&#305;.<br/>";
        }
        if ( !isset( $_POST['text'] ) || trim( $_POST['text'] ) == "" )
        {
            $errormesaj .= "Mesaj yazmam&#305;s&#305;n&#305;z.<br/>\n";
        }
		if ( !$errormesaj )
        {
            $file = file( "file/dat_folder/forum_confiq.dat" );
            ...
            else if ( $dat_skecersiz == 3 )
            {
                $ballazim = 0;
            }
            else if ( $dat_skecersiz == 4 )
            {
                $ballazim = 1;
            }
            else
            ...
            if ( $ballazim == 1 )
            {
                ...
                {
                    $deyishenfiled = "Post";
                    $qeyd = "<br/><u>Sizin hesab&#305;n&#305;zdan Post c&#305;x&#305;lmayacaq sadece postu {$dat_sdeyer}  &#231;ox olan istifade&#231;iler fikir bildire biler.</u>";
                }
                else
                {
                    $deyishenfiled = "bal";
                ...
                echo "Fikir bildirmek &#252;&#231;&#252;n hesab&#305;n&#305;zda \"<b>{$dat_sdeyer} {$deyishenfiled}</b>\" olmal&#305;d&#305;r.{$qeyd}<br/>----<br/>";
                echo "<a href=\"forum.php?{$mygetname}&amp;cmd={$cmd}&amp;uid={$uid}&amp;page={$page}&amp;ref={$ref}\">Geri qay&#305;</a><br/>\n";
                break;
            }
            $time = time( );
            ...
            if ( @( $reg_forum_time < time( ) ) )
            {
                ...
                $text = $_POST['text'];
                $tema1 = $_POST['tema'];
                //$text = substr( @"SELECT * FROM `sh_tem` WHERE `id`='{$uid}';", 0, 550 );
                require( "file/require/sh_files" );
                function shch( $msg )
                {
                    $msg = substr( $msg, 0, 1100 );
                ...
                {
                    $st1 = substr( $text, 0, $minpos + strlen( $smiles[$nm] ) );
                    $st2 = substr( $text, $Tmp_1739, strlen( $text ) - strlen( $st1 ) );
                    $st1 = str_replace( $smiles[$nm], $replaces[$nm], $st1 );
                    $text = $st1.$st2;
                }
                unset( $smiles );
                ...
                fwrite( $file, time( ) + $time_sec );
                fclose( $file );
                $reg_forum_time = time( ) + $time_sec;
                $insert_post = mysql_query( "INSERT INTO `sh_post` SET `avtor`='{$id}', `date`='{$date}', `text`='{$text}', `tema`='{$tema1}', `time`='{$time}';" );
                if ( $insert_post )
                {
                    $mesajadd .= "Mesaj&#305;n&#305;z elave edildi.<br/>";
                    mysql_query( "UPDATE `sh_podcat` SET `post`='{$plus_post_podcat}' WHERE id={$tema_q['cat']}" );
                    mysql_query( "UPDATE `sh_tem` SET `time`='{$time}' WHERE id='{$tema1}';" );
                    $mesajadd .= mysql_error( );
                }
                else
                {
                    $mesajadd .= "Mesaj elave edilmedi yeniden qeyd edin...<br/>".mysql_error( );
                }
            }
            else
            {
                $time_sec = $reg_forum_time - time( );
    ...
    if ( isset( $_GET['edit1'] ) && $forum_level == 3 )
    {
        ...
        {
            echo "M&#246;vzunun ad&#305;<br/><input name=\"name{$ref}\" maxlength=\"50\" value=\"{$edit_tema['name']}\" emptyok=\"true\"/><br/>";
            echo "M&#246;vzu<br/><input name=\"editmovzu{$ref}\" maxlength=\"500\" value=\"".$post['text']."\" emptyok=\"true\"/><br/>";
            echo "<anchor>Deyi&#351;dir<go href=\"forum.php?{$mygetname}&amp;cmd=4&amp;uid={$uid}&amp;edit1={$edit1}&amp;ref={$ref}\" method=\"post\">\r\n<postfield name=\"name\" value=\"$(name{$ref})\"/>\r\n<postfield name=\"editmovzu\" value=\"$(editmovzu{$ref})\"/>\r\n</go></anchor><br/>\n";
        }
        else if ( trim( $_POST['name'] ) != "" )
        {
            if ( strlen( $_POST['editmovzu'] ) < 10 )
            {
                $error = "M&#246;vzunu &#231;ox q&#305;sa yazm&#305;s&#305;z, R&#252;tbeli olduqunuz &#252;&#231;&#252;n 10 simvoldan &#231;ox yazmal&#305;s&#305;z!<br/>";
            }
            if ( 60 < strlen( $_POST['name'] ) )
            {
                $error .= "M&#246;vzunu ad&#305; 50 herfden &#231;ox olmamal&#305;d&#305;r!<br/>";
	...
	if ( isset( $_GET['edit'] ) && $forum_level == 3 )
    {
        $edit = abs( $_GET['edit'] );
        ...
        if ( !isset( $_POST['editmesaj'] ) )
        {
            ...
            echo "Mesaj<br/><input name=\"editmesaj{$ref}\" maxlength=\"300\" value=\"{$edit_tema['text']}\" emptyok=\"true\"/><br/>";
            echo "<anchor>Deyi&#351;dir<go href=\"forum.php?{$mygetname}&amp;cmd=4&amp;uid={$uid}&amp;edit={$edit}&amp;ref={$ref}\" method=\"post\">\r\n<postfield name=\"editmesaj\" value=\"$(editmesaj{$ref})\"/>\r\n</go></anchor><br/>\n";
        }
        else if ( trim( $_POST['editmesaj'] ) != "" )
        {
            ...
            $text = shch( narmobil( $_POST['editmesaj'] ) );
            require( "smile.php" );
            $minpos = 355;
            $nm = 355;
            $j = 0;
    ...
    if ( isset( $_GET['del1'] ) && $forum_level == 3 )
    {
        $uid = abs( $_GET['uid'] );
        if ( !( $tema = mysql_fetch_array( mysql_query( "SELECT * FROM `sh_tem` WHERE `id`='{$uid}';" ) ) ) )
        {
    ...
    {
        if ( isset( $_GET['del'] ) && 2 <= $forum_level )
        {
        ...
        if ( isset( $_GET['cl'] ) && 2 <= $forum_level )
        {
            $uid = $_GET['uid'];
            if ( !( $tema = mysql_fetch_array( mysql_query( "SELECT * FROM `sh_tem` WHERE `id`='{$uid}';" ) ) ) )
            {
            ...
            if ( $_GET['cl'] == 1 )
            {
                if ( mysql_query( "UPDATE `sh_tem` SET `close`='1' WHERE `id`='{$uid}';" ) )
                {
                    echo "M&#246;vzu ba&#287;land&#305;<br/>\n";
                }
            }
            else if ( mysql_query( "UPDATE `sh_tem` SET `close`='0' WHERE `id`='{$uid}';" ) )
            {
                echo "M&#246;vzu a&#231;&#305;ld&#305;<br/>\n";
            }
            echo "----<br/><a href=\"forum.php?{$mygetname}&amp;cmd={$cmd}&amp;uid={$uid}&amp;ref={$ref}\">Geri qay&#305;t</a><br/>----<br/>\n";
            echo "<a href=\"forum.php?{$mygetname}&amp;ref={$ref}\">Forum</a> |\n";
            echo $fsize2;
    ...
    if ( $posts != 0 )
    {
        ...
        while ( $post = mysql_fetch_array( $post_q ) )
        {
            ...
            if ( $i == 1 )
            {
                echo $fsize2."</p><p align=\"center\"><b><u>{$tema['name']}</u></b><br/>------</p><p>{$fsize1} ".$post['text']."<br/>------<br/>\n";
                ...
                if ( $forum_level == 3 )
                {
                    echo "<br/>[<a href=\"forum.php?{$mygetname}&amp;cmd=4&amp;uid={$uid}&amp;edit1={$post['id']}&amp;ref={$ref}\">Edit</a>] [<a href=\"forum.php?{$mygetname}&amp;cmd=4&amp;uid={$uid}&amp;del1&amp;ref={$ref}\">Delete</a>]\n";
                }
                echo "<br/>=============<br/>\n";
            }
            else
            {
                ...
                if ( $forum_level == 3 )
                {
                    echo "[<a href=\"forum.php?{$mygetname}&amp;cmd=4&amp;uid={$uid}&amp;page={$page}&amp;edit={$post['id']}&amp;ref={$ref}\">E</a>]";
                }
                if ( 2 <= $forum_level )
                {
                    echo "[<a href=\"forum.php?{$mygetname}&amp;cmd=4&amp;uid={$uid}&amp;page={$page}&amp;del={$post['id']}&amp;ref={$ref}\">X</a>]\n";
                }
                $post['date'] = str_replace( date( "d.m.Y" ), "Bu g&#252;n", $post['date'] );
        ...
        if ( 0 < $page - 3 )
        {
            $page3left = " <a href=\"".$url_for_pstr.( $page - 3 )."&amp;ref={$ref}\">".$Tmp_2501."</a> | ";
        }
        if ( 0 < $page - 2 )
        {
            $page2left = " <a href=\"".$url_for_pstr.( $page - 2 )."&amp;ref={$ref}\">".( $page - 2 )."</a> | ";
        ...
        if ( 0 < $page - 1 )
        {
            $nazad = "<a href=\"".$Var_63192.( $page - 1 )."&amp;ref={$ref}\">Evvelki</a>";
        }
        if ( $Var_63408 + 1 <= $total )
        {
            $vpered = "<a href=\"".$url_for_pstr.( $page + 1 )."&amp;ref={$ref}\">Sonrak&#305;</a>";
        }
        if ( 1 < $total )
        {
            ...
            echo $pervpage.$page5left.$page4left.$page3left.$page2left.$page1left."<b>".$page."</b>".$page1right.$page2right.$page3right.$page4right.$Var_64464.$nextpage."<br/>".$nazad." ".$vpered;
            echo "<br/>----<br/>\n";
        }
    }
    ...
    if ( $colse != 1 )
    {
        if ( $mesajadd != "" )
        {
            $mesajadd = "<i>".$mesajadd."</i>";
        }
        if ( $tema['close'] == 0 )
        {
            echo "<br/>".$mesajadd." Mesaj yazmaq<br/><input name=\"text{$ref}\" maxlength=\"300\" value=\"{$valuemesaj}\" emptyok=\"true\"/><br/>\r\n<anchor>G&#246;nder<go href=\"forum.php?{$mygetname}&amp;cmd=4&amp;uid={$uid}&amp;page={$page}&amp;ref={$ref}\" method=\"post\">\r\n<postfield name=\"tema\" value=\"{$uid}\"/>\r\n<postfield name=\"text\" value=\"$(text{$ref})\"/>\r\n</go></anchor><br/>----<br/>\n";
        }
        else if ( $tema['close'] == 1 )
        {
            echo "<b>Bu m&#246;vzu ba&#287;l&#305;d&#305;r.</b><br/>----<br/>\n";
        }
        if ( $tema['close'] == 0 && 2 <= $forum_level )
        {
            echo "<a href=\"forum.php?{$mygetname}&amp;cmd=4&amp;uid={$uid}&amp;cl=1&amp;ref={$ref}\">[M&#246;vzunu ba&#287;la]</a><br/>----<br/>\n";
        }
        else if ( $tema['close'] == 1 && 2 <= $forum_level )
        {
            echo "<a href=\"forum.php?{$mygetname}&amp;cmd=4&amp;uid={$uid}&amp;cl=0&amp;ref={$ref}\">[M&#246;vzunu a&#231;]</a><br/>----<br/>\n";
        }
    }
    echo "<a href=\"forum.php?{$mygetname}&amp;ref={$ref}\">Forum</a> \n";
    if ( $podcat && $cat )
    {
        echo "&#0187; <a href='forum.php?{$mygetname}&amp;cmd=2&amp;uid={$cat['id']}&amp;ref={$ref}'>{$cat['name']}</a> &#0187; <a href='forum.php?{$mygetname}&amp;cmd=3&amp;uid={$podcat['id']}&amp;ref={$ref}'>{$podcat['name']}</a><br/>";
    }
...
case "f1" :
        ...
        case "mov" :
            if ( !isset( $_POST['edit'] ) )
            {
                ...
                require( "file/fun/16" );
                echo $fsize1;
                echo "<anchor>[Yenile]<go href=\"forum.php?id={$id}&amp;ps={$ps}&amp;cmd=mov&amp;ref={$ref}\" method=\"post\">\n";
                $j = 1;
                while ( $j <= $i )
                {
                    $f = $j - 1;
        ...
        case "deaktiv" :
            ...
            if ( isset( $_GET['edit1'] ) && $forum_level == 3 )
            {
                ...
                if ( !( $edit_tema = mysql_fetch_array( mysql_query( "SELECT * FROM `sh_tem` WHERE `id`='{$uid}';" ) ) ) )
                {
                    ...
                    echo "{$mygetname}&amp;ref={$ref}\">Forum</a><br/>\n";
                    ob_end_flush( );
                    break;
                }
                $post_q = mysql_query( "SELECT * FROM `sh_post` WHERE `id`='{$edit1}'  LIMIT 1;" );
                ...
                if ( !isset( $_POST['editmovzu'] ) )
                {
                    ...
                    $post['text'] = preg_replace( "|\"/>+|isU", "", $post['text'] );
                    echo "M&#246;vzunun ad&#305;<br/><input name=\"name{$ref}\" maxlength=\"50\" value=\"{$edit_tema['name']}\" emptyok=\"true\"/><br/>";
                    echo "M&#246;vzu<br/><input name=\"editmovzu{$ref}\" maxlength=\"1000\" value=\"".$post['text']."\" emptyok=\"true\"/><br/>";
                    echo "<anchor>Deyi&#351;dir<go href=\"forum.php?{$mygetname}&amp;cmd={$cmd}&amp;uid={$uid}&amp;edit1={$edit1}&amp;ref={$ref}\" method=\"post\">\r\n<postfield name=\"name\" value=\"$(name{$ref})\"/>\r\n<postfield name=\"editmovzu\" value=\"$(editmovzu{$ref})\"/>\r\n</go></anchor><br/>\n";
                }
                else if ( trim( $_POST['name'] ) != "" )
                {
                    if ( strlen( $_POST['editmovzu'] ) < 10 )
                    ...
                    unset( $smiles );
                    unset( $replaces );
                    if ( mysql_query( "UPDATE `sh_tem` SET `name`='{$name}' WHERE `id`='{$uid}';" ) && mysql_query( "UPDATE `sh_post` SET `text`='".$text."' WHERE `id`='{$edit1}';" ) )
                    {
            ...
            $podcat = mysql_fetch_array( mysql_query( "SELECT * FROM `sh_cat` WHERE id='{$podcat['refid']}';" ) );
            $cat = mysql_fetch_array( mysql_query( "SELECT * FROM `sh_cat` WHERE id='{$podcat['refid']}';" ) );
            $i = 1;
            $num = 8;
            @$page = @$_GET['uid'];
            ...
            if ( $posts != 0 )
            {
                ...
                while ( $post = mysql_fetch_array( $post_q ) )
                {
                    ...
                    if ( $i == 1 )
                    {
                        ...
                        $post['date'] = str_replace( date( "d.m.Y" ), "Bu g&#252;n", $post['date'] );
                        echo "<br/>Tarix: ({$post['date']})\n";
                        echo "<br/>[<a href=\"forum.php?&amp;cmd={$cmd}&amp;uid={$uid}&amp;edit1={$post['id']}&amp;ref={$ref}\">Edit</a>] [<a href=\"forum.php?{$mygetname}&amp;cmd={$cmd}&amp;uid={$uid}&amp;del1&amp;ref={$ref}\">Delete</a>]\n";
                ...
                if ( 0 < $page - 5 )
                {
                    $page5left = " <a href=\"".$url_for_pstr.$Tmp_3383."&amp;ref={$ref}\">".( $page - 5 )."</a> | ";
                }
                if ( 0 < $page - 4 )
                {
                    $page4left = " <a href=\"".$url_for_pstr.( $page - 4 )."&amp;ref={$ref}\">".( $page - 4 )."</a> | ";
                ...
                if ( $page + 1 <= $total )
                {
                    $vpered = "<a href=\"".$url_for_pstr.( $page + 1 )."&amp;ref={$Var_85800}\">Sonrak&#305;</a>";
                }
                if ( 1 < $total )
                {
                    echo "----<br/>\n";
        ...
        case "silinib" :
            ...
            if ( isset( $_GET['edit1'] ) )
            {
                ...
                echo "{$mygetname}&amp;ref={$ref}\">Forum</a> |\n";
                echo $fsize2;
                echo "</p>\n";
                echo "</card>\n";
                echo "</wml>\n";
            ...
            if ( isset( $_GET['del1'] ) )
            {
                ...
                if ( !( $tema = mysql_fetch_array( mysql_query( "SELECT * FROM `sh_tem` WHERE `id`='{$uid}' and `tesdiq`='3';" ) ) ) )
                {
                    ...
                    echo "<a href=\"forum.php?{$mygetname}&amp;ref={$ref}\">Forum</a> |\n";
                    echo "<a href=\"forum.php?{$mygetname}&amp;ref={$ref}";
                    echo $fsize2;
                    echo "</p>\n";
                    echo "</card>\n";
                    echo "</wml>\n";
            ...
            $num = 8;
            @$page = @$_GET['uid'];
            $result00 = mysql_query( "SELECT COUNT(*) FROM `sh_post` where `tema`='';" );
            $temp = mysql_fetch_array( $result00 );
            $posts = $temp[0];
            $total = ( $posts - 1 ) / $num + 1;
            $total = intval( $total );
    ...
    case "delete" :
        ...
        if ( $uid )
        {
            ...
            {
                echo "Bu Kataloqda Silinmi&#351; M&#246;vzu yoxdur.<br/>\n";
            }
        }
        else
        {
    ...
    case "fq" :
        ...
        if ( !isset( $_POST['fdeyer'] ) )
        {
            ...
            echo $fsize2;
            echo "<input type=\"text\" name=\"fdeyer{$ref}\" maxlength=\"15\" value=\"{$dat_fdeyer}\"/>\n";
            echo "<select name=\"fbalpost{$ref}\">";
            if ( $dat_fbalpost == 1 )
            {
            ...
            echo "</select>\n";
            echo "<select name=\"fkecersiz{$ref}\">";
            if ( $dat_fkecersiz == 1 )
            {
                echo "<option value=\"1\">F-R ke&#231;ersiz</option>";
                echo "<option value=\"2\">F-R ve C-R ke&#231;ersiz</option>";
                echo "<option value=\"3\">Herkese ke&#231;ersiz</option>";
                echo "<option value=\"4\">Herkese ke&#231;erli</option>";
            }
            else if ( $dat_fkecersiz == 2 )
            {
                echo "<option value=\"2\">F-R ve C-R ke&#231;ersiz</option>";
            ...
            echo $fsize2;
            echo "<input type=\"text\" name=\"sdeyer{$ref}\" maxlength=\"15\" value=\"{$dat_sdeyer}\"/>\n";
            echo "<select name=\"sbalpost{$ref}\">";
            if ( $dat_sbalpost == 1 )
            {
                echo "<option value=\"1\">Bal</option>";
                echo "<option value=\"2\">Post</option>";
            ...
            echo "</select>\n";
            echo "<select name=\"skecersiz{$ref}\">";
            if ( $dat_skecersiz == 1 )
            {
                echo "<option value=\"1\">F-R ke&#231;ersiz</option>";
                echo "<option value=\"2\">F-R ve C-R ke&#231;ersiz</option>";
            ...
            echo "Tesdiqleme<br/>\n";
            echo $fsize2;
            echo "<select name=\"ftesdiq{$ref}\">";
            if ( $dat_ftesdiq == 1 )
            {
                echo "<option value=\"1\">Tesdiq olsun</option>";
                echo "<option value=\"2\">Tesdiq olmas&#305;n</option>";
            }
            else
            {
                echo "<option value=\"2\">Tesdiq olmas&#305;n</option>";
            ...
            echo "</select><br/>";
            echo $fsize1;
            echo "<anchor>[Yenile]<go href=\"forum.php?id={$id}&amp;ps={$ps}&amp;cmd=fq&amp;ref={$ref}\" method=\"post\">\n";
            echo "<postfield name=\"fdeyer\" value=\"$(fdeyer{$ref})\"/>\n";
            echo "<postfield name=\"fbalpost\" value=\"$(fbalpost{$ref})\"/>\n";
            echo "<postfield name=\"fkecersiz\" value=\"$(fkecersiz{$ref})\"/>\n";
            echo "<postfield name=\"sdeyer\" value=\"$(sdeyer{$ref})\"/>\n";
            echo "<postfield name=\"sbalpost\" value=\"$(sbalpost{$ref})\"/>\n";
            echo "<postfield name=\"skecersiz\" value=\"$(skecersiz{$ref})\"/>\n";
            echo "<postfield name=\"ftesdiq\" value=\"$(ftesdiq{$ref})\"/>\n";
            echo "</go></anchor><br/>\n";
        }
        else
        {
            ...
            else if ( !preg_match( "!^[0-9]+$!i", $fbalpost ) )
            {
                $error = $error_mesaj;
            }
            else if ( !preg_match( "!^[0-9]+$!i", $fkecersiz ) )
            {
                $error = $error_mesaj;
            }
            else if ( !preg_match( "!^[0-9]+$!i", $sdeyer ) )
            {
                $error = $error_mesaj;
            }
            else if ( !preg_match( "!^[0-9]+$!i", $sbalpost ) )
            {
                $error = $error_mesaj;
            }
            else if ( !preg_match( "!^[0-9]+$!i", $skecersiz ) )
            {
                $error = $error_mesaj;
            }
            else if ( !preg_match( "!^[0-9]+$!i", $ftesdiq ) )
            {
                $error = $error_mesaj;
            }
            if ( $error )
    ...
    case "f2" :
        ...
        else if ( isset( $_GET['edit'] ) )
        {
            ...
            if ( !isset( $_POST['name'] ) )
            {
                ...
                echo $fsize2;
                echo "<input name=\"name{$ref}\" value=\"".$edit_cat['name']."\" maxlength=\"25\" emptyok=\"true\"/><br/>\n";
                echo $fsize1;
                echo "<anchor>Deyi&#351;dir<go href=\"forum.php?{$mygetname}&amp;cmd=f2&amp;edit={$uid}&amp;ref={$ref}\" method=\"post\">\r\n<postfield name=\"name\" value=\"$(name{$ref})\"/>\r\n</go></anchor><br/>\n";
            }
            else
            {
                if ( trim( $_POST['name'] ) != "" )
                {
                    ...
                    if ( $upd_cat )
                    {
                        echo "B&#246;lmenin ad&#305; deyi&#351;dirildi<br/>\n";
                    }
                    else
                    {
                        echo mysql_error( )."<br/>Xeta ba&#351; verdi<br/>\n";
                    }
                }
            }
        }
    ...
    case "f3" :
        ...
        if ( isset( $_GET['new'] ) )
        {
            ...
            if ( !( $new_forum = mysql_fetch_array( mysql_query( "SELECT * FROM `sh_cat` WHERE `id`='{$uid}';" ) ) ) )
            {
                ...
                echo $Tmp_5531."<a href=\"forum.php?{$mygetname}&amp;cmd=f1&amp;ref={$ref}\">Forum Panel</a><br/>\n";
                break;
            }
            if ( !isset( $_POST['name'] ) )
            {
                ...
                echo $fsize2;
                echo "<input name=\"name{$ref}\" maxlength=\"25\" emptyok=\"true\"/><br/>\n";
                echo $fsize1;
                echo "<anchor>Yarat<go href=\"forum.php?{$mygetname}&amp;cmd=f3&amp;new={$uid}&amp;ref={$ref}\" method=\"post\">\r\n<postfield name=\"name\" value=\"$(name{$ref})\"/>\r\n</go></anchor><br/>\n";
            }
            else
            {
                if ( trim( $_POST['name'] ) != "" )
        ...
        else if ( isset( $_GET['edit'] ) )
        {
            ...
            if ( $_POST['name'] == "" )
            {
                echo "Kataloq<br/>\n";
                echo $fsize2;
                echo "<input name=\"name{$ref}\" value=\"{$podcat['name']}\" maxlength=\"25\" emptyok=\"true\"/><br/>\n";
                echo $fsize1;
                echo "<anchor>Deyi&#351;dir<go href=\"forum.php?{$mygetname}&amp;cmd=f3&amp;edit={$uid}&amp;ref={$ref}\" method=\"post\">\r\n<postfield name=\"name\" value=\"$(name{$ref})\"/>\r\n</go></anchor><br/>\n";
            }
            else
            {
                require( "file/require/sh_files" );
                ...
                if ( $upd_podcat )
                {
                    echo "Kataloqun ad&#305; deyi&#351;dirildi.<br/>\n";
                }
                else
                {
                    echo "Xeta<br/>".mysql_error( )."<br/>\n";
    ...
    {
        $uid = abs( $_GET['del'] );
        if ( !( $del_cat = ( mysql_query( "SELECT * FROM `sh_podcat` WHERE `id`='{$uid}';" ) ) ) )
        {
            echo "Kataloq tap&#305;lmad&#305;.<br/>----<br/>";
            echo "<a href=\"forum.php?{$mygetname}&amp;cmd=f1&amp;ref={$ref}\">Forum Panel</a><br/>\n";
            break;
...
case "f5" :
    ...
    if ( $_POST['name'] == "" )
    {
        ...
        echo $fsize2;
        echo "<input name=\"name{$ref}\" maxlength=\"100\" emptyok=\"true\"/><br/>\n";
        echo $fsize1;
        echo "<u>Text S&#246;z</u><br/>\n";
        echo $fsize2;
        echo "<input name=\"desc{$ref}\" maxlength=\"1000\" emptyok=\"true\"/><br/>\n";
        echo $fsize1;
        echo "<i>Xeberin mezmunu</i><br/>\n";
        echo $fsize2;
        echo "<input name=\"text{$ref}\" maxlength=\"400\" emptyok=\"true\"/><br/>\n";
        echo $fsize1;
        echo "<anchor>Elave et<go href=\"forum.php?{$mygetname}&amp;cmd=f5&amp;ref={$ref}\" method=\"post\">\r\n<postfield name=\"name\" value=\"$(name{$ref})\"/>\r\n<postfield name=\"desc\" value=\"$(desc{$ref})\"/>\r\n<postfield name=\"text\" value=\"$(text{$ref})\"/>\r\n</go></anchor><br/>\n";
    }
    else if ( ( $_POST['name'] ) != "" )
    {
        if ( strlen( $_POST['name'] ) < 3 )
        {
            $error .= "Xeberin ad&#305;n&#305 &#231;ox q&#305;sa yazm&#305;s&#305;z!<br/>\n";
        }
        if ( strlen( $_POST['text'] ) < 10 )
        {
            $error .= "Xeberin mezmununu &#231;ox q&#305;sa yazm&#305;s&#305;z!<br/>\n";
        }
        if ( !$error )
        {
            require( "file/require/sh_files" );
            ...
            $date = date( "d.m.Y H:i" );
            $avtor = $row['id'];
            if ( mysql_query( "INSERT INTO `sh_new` SET `name`='".$name."', `description`='".$desc."', `text`='".$text."', `time`='{$time}', `date`='{$date}', `avtor`='{$avtor}';" ) )
            {
                echo "Xeber elave edildi.".mysql_error( )."<br/>\n";
            }
            else
            {
                echo "Xeta".mysql_error( )."<br/>\n";
        ...
        else
        {
            ...
            echo $fsize2;
            echo "<input name=\"name{$ref}\" value=\"{$_POST['name']}\" maxlength=\"100\" emptyok=\"true\"/><br/>\n";
            echo $fsize1;
            echo "<u>Text S&#246;z</u><br/>\n";
            echo $fsize2;
            echo "<input name=\"desc{$ref}\" value=\"{$_POST['desc']}\" maxlength=\"1000\" emptyok=\"true\"/><br/>\n";
            echo $fsize1;
            echo "<i>Xeberin mezmunu</i><br/>\n";
            echo $fsize2;
            echo "<input name=\"text{$ref}\" value=\"{$_POST['text']}\" maxlength=\"400\" emptyok=\"true\"/><br/>\n";
            echo $fsize1;
            echo "<anchor>Elave et<go href=\"forum.php?{$mygetname}&amp;cmd=f5&amp;ref={$ref}\" method=\"post\">\r\n<postfield name=\"name\" value=\"$(name{$ref})\"/>\r\n<postfield name=\"desc\" value=\"$(desc{$ref})\"/>\r\n<postfield name=\"text\" value=\"$(text{$ref})\"/>\r\n</go></anchor><br/>\n";
        }
    }
    ...
    if ( $cid )
    {
        echo "<a href =\"forum.php?id={$id}&amp;ps={$ps}&amp;ref={$ref}\">Forum</a><br/>*****<br/>\n";
    }
